...
// Function prototype to get Cli Record Pointer from bottom of file.
//    If access from other files is requred, use a non-static function
//    with more descriptive name and put prototype in the header-file.
static const t_cli_record *CRP();
...
void DefaultDemoSetup()
{
   ...
   //    CRP() is a trick to get a pointer to CliRecords at the bottom.
   //    "X-Y-Z..." is a descriptive name, shown with help ('?' and '? xyz').
   //    'xyz' is the keyword that precedes all commands in the menu.
   //    Vars in the registry that start with 'X_Y_Z.' are shown with help ('? xyz').
   //    SetHelpText for registry vars is optional.
   CliAddCommands(CRP(), "X-Y-Z Demo Menu", "xyz", "X_Y_Z.");

   Registry.Add("X_Y_Z.XyzDemoVar", XyzDemoVar);
   Registry.SetHelpText("X_Y_Z.XyzDemoVar", "Just a demo variable");
}
...
static const t_cli_record *CRP() { return CliRecords; }
// Copy to here & update
