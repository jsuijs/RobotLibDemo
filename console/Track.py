...
class MyScreen(tk.Frame):     
   ...
   def __init__(self, master):
      ...
      # create objects
      self.canvas = tk.Canvas(master, width=ConfigData['WinSizeX']*ConfigData['WinScale'], height=ConfigData['WinSizeY']*ConfigData['WinScale'], background="bisque")  
      ...
      # scale back...
      self.canvas.scale("all", 0, 0, ConfigData['WinScale'], ConfigData['WinScale'])  
      self.ScaleFactor *= ConfigData['WinScale']
...
if __name__ == "__main__":   
   ...
   try:
      with open('TrackCfg.json', 'r') as fp:
         ConfigData = json.load(fp)

   except:   
      # no config => load defaults and & save (template) config file
      ConfigData = {  'WinSizeX'  : 3800,          # window size in mm
                      'WinSizeY'  : 2600,          # window size in mm
                      'WinScale'  : 0.3}           # zoomfactor applied at startup
      with open('TrackCfg.json', 'w') as fp:
   ...
   RLCommon.MQttClient(RLCommon.on_message) # put payload of messages in RLCommon.MsgQueue

   def DataTakt():
      UpdatePosition = 0
      if len(RLCommon.MsgQueue) > 0 : 
